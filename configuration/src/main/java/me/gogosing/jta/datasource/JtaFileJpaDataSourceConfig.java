...
public class JtaFileJpaDataSourceConfig extends JtaDataSourceConfig {
	...
	public DataSource filePersistenceXaDataSource(
		final @Qualifier(FILE_PERSISTENCE_XA_PROPERTIES) Properties filePersistenceXaProperties
	) {
		AtomikosDataSourceBean dataSource = new AtomikosDataSourceBean();

		dataSource.setPoolSize(FILE_PERSISTENCE_XA_DATA_SOURCE_POOL_SIZE);
		dataSource.setUniqueResourceName(FILE_PERSISTENCE_UNIQUE_RESOURCE_NAME);
		dataSource.setXaDataSourceClassName(FILE_XA_DATA_SOURCE_CLASS_NAME);
		dataSource.setXaProperties(filePersistenceXaProperties);
	...
	public LocalContainerEntityManagerFactoryBean xaEntityManagerFactory(
		final @Qualifier(FILE_PERSISTENCE_XA_DATA_SOURCE) DataSource filePersistenceXaDataSource
	) {
		LocalContainerEntityManagerFactoryBean entityManager = new LocalContainerEntityManagerFactoryBean();

		entityManager.setDataSource(filePersistenceXaDataSource);
		entityManager.setJpaVendorAdapter(jpaVendorAdapter);
		entityManager.setPackagesToScan(FILE_PERSISTENCE_PACKAGE);
		entityManager.setPersistenceUnitName(FILE_PERSISTENCE_UNIT);
	...
	private Map<String, Object> getJpaPropertyMap() {
		Map<String, Object> properties = filePersistenceHibernateProperties()
